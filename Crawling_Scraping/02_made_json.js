...
require("puppeteer"); 인형조종사를 의미하는 단어
                      ...
                      웹 브라우저 자동화와 관련된 모든 기능을 사용할 수 있음
...
async - await : 
                ...
                await : 시간이 걸리는 작업이 끝날 때 까지 기다리라고 지시하는 것
                        ...
                        async 내부에서만 사용 가능
                        await 를 만나는 코드는 결과값이 완료되어 나올 때 까지 함수의 실행을 일시 중지하고 기다림
*/
async function scrapeQuotes() {
  // 1. 브라우저 실행 puppeteer.launch = 구글 크롬 배경으로 새로운 크롬을 시작하겠다.
  // headless : "new" 웹 사이트를 화면에 보이지 않는 상태에서 실행
  // headless : false 웹 사이트에서 데이터 수집하는 화면을 개발자가 볼 수 있는 상태로 실행
  ...
  // 다음 작업은 시작하지 않고 기다린다.
  // .quote 내부에 작성된 데이터를 0 번부터 순차적으로 작성하기 위해
  // 동기작업으로 파일 쓰기를 진행
  ...
  JSON.stringify       : 객체나 배열을 문자열 형태로 변환
                quotes : 객체나 배열 대상으로 자바스크립트 변수이름에 저장되어 있는
                  ...
                  null : 데이터를 문자열로 변환하기 전에 특정 규칙에 따라 값을 바꾸거나
                     ...
                     2 : 변환된 JSON 의 문자열을 사람이 보기 좋게 만들어주는 들여쓰기 간격
  ...
  */
  ...
  // 6. 브라우저 닫기 = 브라우저를 닫기 싫으면 안해도 됨, 닫지 않으면 메모리 누수 발생 (닫아주는게 좋음)
  await browser.close();
}
