...
selectBtn.addEventListener("click", function () {
  /*
        Math = JS에서 수학적 계산을 위해 제공되는 내장 객체
                * 내장 객체 : JS 개발팀이 추가적으로 만든 객체 기능
                             따로 설치할 필요가 없이 맨 첫글자를 대문자로 작성

            메서드(기능들)
            .random()     : 0 이상 1 미만의 랜덤한 소수를 생성
            .floor(숫자)  : 소수점을 버림하여 정수로 만듬(내림 처리)
            .ceil(숫자)   : 소수점을 올림하여 정수로 만듬(올림 처리)
            .round(숫자)  : 소수점을 반올림하여 5 부터는 올림, 4 이하는 내림 처리
  ...
  // .random() = 0 이상 1 미만의 랜덤한 소수
  // menus.length = 15
  // 소수값 * 15개의 메뉴 = 정수.소수
  // 나온 정수.소수에서 나머지 소수는 모두 버림
  // index 의 값이 마지막 자리수에서 -1 하기 때문에 14
