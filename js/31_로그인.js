...
function loginFn() {
  ...
  // 문자열을 객체 형태로 변환해서 storedUser 에 담아놓기
  const storedUser = JSON.parse(storedUserJSON);

  // 4. 입력한 이메일과 저장된 이메일을 비교하고
  // 참고 : 추후에는 DB 에 저장된 이메일 값과 사용자가 서버로 보낸 이메일 값이
  //        일치 여부에 대해서는 백엔드에서 작업처리를 진행한 후 프론트엔드로 전달
  //        연결된 DB가 존재하지 않기 때문에 프론트엔드에서 모든 작업을 진행
  if (userEmail === storedUser.email) {
    ...
    // 웹 사이트 session 에 자바스크립트에서 작성한 로그인 정보를 저장
