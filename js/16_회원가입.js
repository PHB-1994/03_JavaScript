...
// 그런데 만약에 모두 다 제대로 안했다면 false 작성 제대로 안함 처리
// 값을 하나씩 제대로 작성할 때마다 true 로 변경해줄것!
const checkList = {
  inputId: false, // input 내부에 개발자가 원하는 대로 아이디값을 제대로 작성했다면 true
  inputPw: false, // input 내부에 개발자가 원하는 대로 비밀번호값 제대로 작성했다면 true
...
inputId.addEventListener("input", (e) => {
  ...
  if (valId.trim().length == 0) {
    // 내부에 아무것도 작성 안한게 맞다면
    idResult.textContent = "영어, 숫자, -, 6 ~ 16글자 사이 작성하세요.";
    e.target.value = "";
  ...
  if (regExpId.test(valId)) {
    ...
    idResult.classList.remove("ckeck");
    checkList["inputId"] = false;
  }
});
...
inputPw.addEventListener("input", (e) => {
  ...
  if (regExpPw.test(valPw)) {
    pwResult.textContent = "유효한 아이디 형식입니다.";
    pwResult.classList.add("check");
    pwResult.classList.remove("error");
    checkList["inputPw"] = true; // 비밀번호 형식이 일치하다면 위 작성한 checkList 에서
    // inputPw 에 해당하는 boolean(true/false) 을 true 로 변경하겠다.
  } else {
    pwResult.textContent = "유효하지 않은 아이디 형식입니다.";
...
function checkPw기능() {
  ...
  // 0 이거나        또는    0 인게 둘중 하나라도 참이라면
  if (pwVal.length == 0 || checkVal.length == 0) {
    pwCheckResult.textContent = "비밀번호를 먼저 입력해주세요.";
    // pwCheckResult.style.background = "배경식"; 2순위 형식의 style css
    // pwCheckResult 곳에 calss 명칭을 추가하거나 재거하겠다. 4순위 style css
    // 태그 명칭 옆에 class 명칭을 지우겠다. check 와 error 를 제거하겟다.
...
inputName.addEventListener("input", (e) => {
  const valName = e.target.value; // input 에서 소비자가 작성한 값 가져와

  if (valName.trim().length == 0) {
    nameResult.textContent = "한글 2 ~ 15(단모음, 단자음 제외)";
...
btn.addEventListener("click", (e) => {
  // 모든 checkList 항복이 true 인지 확인하고 true가 아니라면 회원가입 불가!!
  // true 라면 회원가입 가능~
  ...
  alert("회원가입이 완료되었습니다.");
  signupForm.onsubmit(); // 회원가입 폼 제출 완료 기능 설정
});
