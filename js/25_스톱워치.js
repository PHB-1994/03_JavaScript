...
// StartBtn 버튼 클릭 시 10ms 마다 display(시간)을 증가 0.01 초

startBtn.addEventListener("click", (e) => {
  // 클릭한 버튼에 작성된 내용이 PAuse
  if (e.target.textContent == "PAUSE") {
    e.target.textContent = "START"; // 버튼 내용 START 로 변경
  ...
  currInterval = window.setInterval(() => {
    count++; // 1초씩 증가
    output(); // 화면에 시간 출력하는 기능 (실습코드에 존재)
  }, 10); // 10ms

  e.target.textContent = "PAUSE"; // 카운터 시작 중에 정지하고 싶으면 정지할 수 있도록 버튼 내부 PAUSE 변경
});
...
li.innerText = display.innertText;
