/* 매개 변수, 전달 인자*/
const input1 = document.getElementById("input1"); // querySelector -> # 을 작성하지 않아도 되는 메서드로 교체
const btn1 = document.getElementById("btn1"); // querySelector -> # 을 작성하지 않아도 되는 메서드로 교체

/* btn1 클릭 시 #input 의 값을 읽어와 print1() 함수를 호출 */
// 자주 사용하는 기능을 분리해서
// 기능 명칭으로 사용할 수 있다!
...
btn3a.addEventListener("click", function () {
  ...
  numbers.push(30); // index 2번 째 자리 다음에 30이 추가
  console.log(numbers);

  numbers.push(pow(2, 4)); // == numbers.push(16);  pow(2,4) 2의 4승
  // push 나 numbers [인덱스번호] 를 이용해서 값을 추가할 때
  // 함수로 만든 기능을 활용해서 숫자나 글자를 추가할 수 있다.
});

// 전달 받은 수를 x제곱해서 반환하는 방식
function pow(num, x) {
  let result1 = 1; // 곱했을 때 결과에 영향을 주지 않기 위해 1 초기화
  for (let i = 0; i < x; i++) {
    result1 *= num;
  }
  return result1;
