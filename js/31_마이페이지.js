$(function () {
  // 1. 타이머를 중지시키기 위한 변수 이름 설정
  let timerInterval;

  // sessionStorage 에서 로그인 정보 가져오기
  const 세션로그인유저 = sessionStorage.getItem("loggedInUser");

  // 로그인 정보가 없으면 로그인 페이지로 보내기
  if (!세션로그인유저) {
    // !세션로그인유저 = 로그인된 유저가 없는게 트루라면
